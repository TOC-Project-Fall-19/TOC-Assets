...
public class palindromescript : MonoBehaviour {
    ...
    public void generateCubes(GameObject[] cubes, int i,string txt) {
        ...
        cubes[i].transform.position = new Vector3(((i * 6) - 5f) + 149.5f, 8.2f, -362.5f);
        ...
        child.transform.position = new Vector3(((i * 6) - 5f) + 149.5f, 8.2f, -362.5f);
    ...
    public void destroyObject(char[] charArr) {
        for (int i = 0; i < (charArr.Length); i++)
        {
            ...
            Instantiate(myPrefab, new Vector3(((i * 6) - 5f) + 149.5f, 8.2f, -362.5f), Quaternion.identity);
